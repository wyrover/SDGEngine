...
namespace SDGEngine
{
	void Viewport::Init(TileMap *mapinfo, int ViewportW, int ViewportH)
	{
	...
	void Viewport::Update(float delta)
	{
		if (m_TileMapInfo)
		{
			...
			{
				if ((m_ViewPort.left + MoveUnit * delta) > m_TotalMapSize.right - m_ViewPort.right) {
					m_ViewPort.left = m_TotalMapSize.right - m_ViewPort.right;
				}
				else {
					m_ViewPort.left += MoveUnit * delta;
				}
			}
			if (Singleton<Input>::GetSingleton()->keyDown(DIK_A))
			{
				if ((m_ViewPort.left - MoveUnit * delta) < m_TotalMapSize.left) {
			...
			{
				if ((m_ViewPort.top + MoveUnit * delta) > m_TotalMapSize.bottom - m_ViewPort.bottom) {
					m_ViewPort.top = m_TotalMapSize.bottom - m_ViewPort.bottom;
				}
				else {
					m_ViewPort.top += MoveUnit * delta;
				}
			}
			if (Singleton<Input>::GetSingleton()->keyDown(DIK_W))
			{
				if ((m_ViewPort.top - MoveUnit * delta) < m_TotalMapSize.top) {
